...
class DbTemplate extends Form {
	...
	*/
	 ...
	 * @return bool
	 */
    public function DeleteMultiID($multiID){ //$multiID = 1,2,10,8
		global $db;
			// Delete the entry
			$query = 'DELETE FROM ' . $this->table . ' WHERE id in('.$multiID.')';
			if($db->Query($query, $this->database)){
				return true;
	...
	public function DeleteAll(){ //$multiID = 1,2,10,8
		global $db;
		// Delete the entry
	...
	public function getValueTable($id,$fields){
		$this->SetPrimary((int)$id);
		$this->GetInfo();
	...
	public function getLastID($field) {
        ...
        $num = "SELECT $field FROM " . $this->table . " ORDER BY $field DESC LIMIT 0,1 ";
		$query = $db->Query($num);
		if($db->NumRows($query) > 0){
        	$rowid = $db->FetchArray($query);
			return $rowid["$field"] ;
		}else{
			return "1";
		}
	...
	public function getLastCode($field,$id_letter) {
		global $db;
		...
		if($db->NumRows($query) > 0){
        	$rowid = $db->FetchArray($query);
			$lenght = strlen($rowid["$field"]);
			$cutzero = ltrim($rowid["$field"],"0");
			$id_num = $cutzero + 1;   //40 + 1
			$id_num = str_pad($id_num, $lenght, "0", STR_PAD_LEFT); //return 000041
			$new_id = $id_letter . $id_num;  
			return $new_id;
		}
    }
	
	public function CountDataDesc($myID, $myWhere) {
		global $db;
        ...
        if ($myID != "") {
            $sql = "SELECT Count($myID) cn FROM " . $this->table . $SqlWhere;
            $Query = $db->Query($sql);
            $RecordCount = $db->NumRows($Query);
            if ($RecordCount > 0) {
	...
	public function SumDataDesc($myID, $myWhere) {
		global $db;
        ...
        if ($myID != "") {
            $sql = "SELECT SUM($myID) cn FROM " . $this->table . $SqlWhere;
            $Query = $db->Query($sql);
            $RecordCount = $db->NumRows($Query);
            if ($RecordCount > 0) {
                $Row = $db->FetchArray($Query);
	...
	public function updateSQL($arrData = "", $arrKey = "") {
		global $db;

        if ($arrData == "")
            $arrData = $this->_data;
        if ($arrKey == "")
            $arrKey = $this->_myWhere;

        foreach ($arrData as $key => $value) {
				//$value2 = mysql_real_escape_string($value);
            if ($value == "NOW()") {
			
                $this->_myUpdate .= $key . " = " . $value . " , ";
            } else {
                $this->_myUpdate .= $key . " = '" . $value . "' , ";
            }
        }
        ...
        foreach ($arrKey as $key => $value) {
            if (count($arrKey) == 1) {
                $this->_myWhereCauseKeyID .= $key . " = '" . $value . "'";
            } else {
                $this->_myWhereCauseKeyID .= ' AND ' . $key . " = '" . $value . "'";
            }
        }
       if (count($arrKey) > 1) {
      
                   
         $this->_myWhereCauseKeyID =  substr($this->_myWhereCauseKeyID , 4);
       }

        $sql = "UPDATE " . $this->table . " SET " . substr($this->_myUpdate, 0, -2) . " WHERE " . $this->_myWhereCauseKeyID . " ";

        $db->Query($sql);
        unset($this->_myUpdate);
        unset($this->_myWhereCauseKeyID);
        unset($this->_data, $this->_myWhere);
		return $sql;
    }
	...
	public function get_enum_values($field){
		...
		$field = $field; // The field that contains the ENUM
		$result = $db->Query('show columns from ' . $this->table . ';');
		while($tuple = $db->FetchArray($result))
		{
	...
	function CreateDataList($myID, $myDesc, $myWhere, $myData, $order = "") {
		global $db;
        ...
        if ($myID != "") {
            $sql = "SELECT $myID,$myDesc FROM " . $this->table . $SqlWhere . $order ;
            $Query = $db->Query($sql) OR DIE("Error: เกิดความผิดพลาด <br>$sql<br>\n");
            $RecordCount = $db->NumRows($Query);
            if ($RecordCount > 0) {
                $index = 1;
                while ($index <= $RecordCount) {
                    $RowGroup = $db->FetchArray($Query);
                    echo "<option value='" . $RowGroup["$myID"] . "'";
                    if ($RowGroup["$myID"] == $myData) {
                        echo " selected ";
                    }
                    echo ">";
                    echo $RowGroup["$myDesc"];
                    echo "</option>";
                    $index++;
                }
            }
	...
	function CreateDataList2($myID, $myDesc) {
		global $db;
	

        if ($myID != "") {
            $sql = "SELECT $myID,$myDesc FROM " . $this->table ;
            $Query = $db->Query($sql) OR DIE("Error: เกิดความผิดพลาด <br>$sql<br>\n");
            $RecordCount = $db->NumRows($Query);
            if ($RecordCount > 0) {
                $index = 1;
	...
	function CreateDataList3($myID, $myDesc, $myWhere = "", $myData, $order = "") {
		global $db;
	...
	function getDataDesc($myID, $myWhere) {
		global $db;
        ...
        if ($myID != "") {
            $sql = "SELECT $myID FROM " . $this->table . $SqlWhere;
            $Query = $db->Query($sql);
            $RecordCount = $db->NumRows($Query);
	...
	function getDataDescLastID($myID, $myWhere) {
		global $db;
        ...
        if ($myID != "") {
            $sql = "SELECT $myID FROM " . $this->table . $SqlWhere . " ORDER BY id DESC LIMIT 1";
            $Query = $db->Query($sql);
            $RecordCount = $db->NumRows($Query);
	...
	public function getBlocks($search) {
        ...
        $num = "SELECT static_blocks FROM " . $this->table . " WHERE static_blocks_ref = '" . $search . "'";
		$query = $db->Query($num);
		if($db->NumRows($query) > 0){
        	$rowid = $db->FetchArray($query);
			return $rowid["static_blocks"];
		}else{
			return "&nbsp;";
		}
    }
