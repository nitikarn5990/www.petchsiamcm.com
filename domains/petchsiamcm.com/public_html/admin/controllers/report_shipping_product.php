...
if ($_POST ['submit_bt'] == 'บันทึกข้อมูล' || $_POST ['submit_bt'] == 'บันทึกข้อมูล และแก้ไขต่อ') {
    ...
    if ($orders->updateSQL($arrData, $arrKey)) {
        ...
        if ($redirect) {
            header('location:' . ADDRESS_ADMIN_CONTROL . 'report_shipping_product');
            die();
        } else {
            header('location:' . ADDRESS_ADMIN_CONTROL . 'report_shipping_product&action=edit&id=' . $_GET['id']);
            die();
        }
    } else {
...
<?php if ($_GET['action'] == "add" || $_GET['action'] == "edit") { ?>
            ...
            <div class="da-panel collapsible">
                ...
                <div class="da-panel-content da-form-container">
                    <form id="validate" enctype="multipart/form-data"
                          action="<?php echo ADDRESS_ADMIN_CONTROL ?>report_shipping_product<?php echo ($orders->GetPrimary() != '') ? '&action=edit&id=' . $orders->GetPrimary() : ''; ?>"
    ...
    ?>
                            <div class="da-form-row">
                                ...
                                <div class="da-form-item large">
                                    <input type="text" name="shipping_cost" id="product_cost"
                                           value="<?php echo ($orders->GetPrimary() != '') ? $orders->getValue('shipping_cost') : ''; ?>"
                                           class="span12 required"  readonly="readonly"/>
                                </div>
                            </div>
                            <div class="da-form-row">
                            ...
                            <div class="da-form-row">
                                <label class="da-form-label">Tracking (หมายเลขติดตาม EMS) / หมายเหตุ<span class="required"></span></label>
                                <div class="da-form-item large">

                                    <textarea class="span12" name="tracking"><?php echo ($orders->GetPrimary() != '') ? $orders->GetValue('tracking') : ''; ?></textarea>
                                </div>
                            </div>

                            <div class="da-form-row <?= $orders->GetValue('shipping_status') == 'รอการส่งสินค้า' ? 'bg-warning' : 'bg-success' ?>">
                                <label class="da-form-label">สถานะ <span class="required">*</span></label>
                                <div class="da-form-item large">
                                    <ul class="da-form-list">
    <?php
    $getStatus = $orders->get_enum_values('shipping_status');
    ...
    foreach ($getStatus as $status) {
        ?>
                                            <li><input type="radio"
                                                       name="shipping_status" id="shipping_status" value="<?php echo $status ?>"
                                            <?php echo ($orders->GetPrimary() != "") ? ($orders->GetValue('shipping_status') == $status) ? "checked=\"checked\"" : "" : ($i == 1) ? "checked=\"checked\"" : "" ?>
                            ...
                            <div class="btn-row">
                                <input type="submit" name="submit_bt" value="บันทึกข้อมูล"
                                       ...
                                       href="<?php echo ADDRESS_ADMIN_CONTROL ?>report_shipping_product"
...
<?php } else { ?>
    ...
    if ($_POST['s_date'] != '') {
        ...
        $sql = "SELECT * FROM " . $orders->getTbl() . " WHERE shipping_status = 'ส่งสินค้าแล้ว' AND (shipping_date BETWEEN '" . $date_start . "' AND '" . $date_end . "' ) ORDER BY id DESC";
    } else {
        $sql = "SELECT * FROM " . $orders->getTbl() . " WHERE shipping_status = 'ส่งสินค้าแล้ว' ORDER BY id DESC";
    }
    ...
    while ($row = $db->FetchArray($query)) {
        ?>
                                <tr>
                                    ...
                                    <td class="center"><?php echo $row['shipping_status']; ?></td>
                                    <td class="center"><?php echo $functions->ShowDateThTime($row['shipping_date']) ?></td>

                                    <td class="center"><a
                                            href="<?php echo ADDRESS_ADMIN_CONTROL ?>report_shipping_product&action=edit&id=<?php echo $row['id'] ?>"
                                            class="btn btn-primary btn-small">แก้ไข / ดู</a> <a href="#"
                                            onclick="if (confirm('คุณต้องการลบข้อมูลนี้หรือใม่?') == true) {
                                                                document.location.href = '<?php echo ADDRESS_ADMIN_CONTROL ?>report_shipping_product&action=del&id=<?php echo $row['id'] ?>'
                                                                        }"
        ...
        function multi_delete() {
            ...
            if (res != '') {
                if (confirm('คุณต้องการลบข้อมูลนี้หรือใม่?') == true) {
                    document.location.href = '<?php echo ADDRESS_ADMIN_CONTROL ?>report_shipping_product&action=del&id=' + res;
                }
            }
