...
if ($_POST ['submit_bt'] == 'บันทึกข้อมูล' || $_POST ['submit_bt'] == 'บันทึกข้อมูล และแก้ไขต่อ') {
    ...
    if ($orders->updateSQL($arrData, $arrKey)) {
        ...
        if ($redirect) {
            ...
            header('location:' . ADDRESS_ADMIN_CONTROL . 'shipping_product');

            die();
        } else {

            header('location:' . ADDRESS_ADMIN_CONTROL . 'shipping_product&action=edit&id=' . $_GET['id']);
            ...
            <div class="da-panel collapsible">
                ...
                <div class="da-panel-content da-form-container">
                    <form id="validate" enctype="multipart/form-data"
                          action="<?php echo ADDRESS_ADMIN_CONTROL ?>shipping_product<?php echo ($orders->GetPrimary() != '') ? '&action=edit&id=' . $orders->GetPrimary() : ''; ?>"
        ...
        ?>
                            ...
                            <div class="btn-row">
                                <input type="submit" name="submit_bt" value="บันทึกข้อมูล"
                                       ...
                                       href="<?php echo ADDRESS_ADMIN_CONTROL ?>shipping_product"
    ...
    while ($row = $db->FetchArray($query)) {
        ?>
                                <tr>
                                    ...
                                    <td class="center"><?= $row['shipping_status'] == 'รอการส่งสินค้า' || $row['shipping_status'] == '' ? 'รอการส่งสินค้า' : 'ส่งสินค้าแล้ว' ?></td>
                                    <td class="center"><?php echo $functions->ShowDateThTime($row['order_date']) ?></td>

                                    <td class="center"><a
                                            href="<?php echo ADDRESS_ADMIN_CONTROL ?>shipping_product&action=edit&id=<?php echo $row['id'] ?>"
                                            class="btn btn-primary btn-small">แก้ไข / ดู</a> <a href="#"
                                            onclick="if (confirm('คุณต้องการลบข้อมูลนี้หรือใม่?') == true) {
                                                        document.location.href = '<?php echo ADDRESS_ADMIN_CONTROL ?>shipping_product&action=del&id=<?php echo $row['id'] ?>'
                                                                }"
        ...
        function multi_delete() {
            ...
            if (res != '') {
                if (confirm('คุณต้องการลบข้อมูลนี้หรือใม่?') == true) {
                    document.location.href = '<?php echo ADDRESS_ADMIN_CONTROL ?>shipping_product&action=del&id=' + res;
                }
            }
